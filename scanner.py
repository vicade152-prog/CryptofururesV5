...
def fetch_coin_detail(inst_id):
    """Live single-symbol detail fetch for the click-to-expand panel.
    Every value here is a fresh, live OKX API call made at request time --
    nothing is cached, estimated, or carried over from the last scan
    (except the OI rolling history, which is read from the same in-memory
    window the scan loop already maintains -- see _OI_HISTORY).

    Returns a dict with raw values and pre-computed pass/fail checks for
    the 3-condition "safe trade" filter:
      1. Funding rate < 0.05% (0.0005 as a decimal)
      2. Price within 1.5% of the live 15m EMA(20)
      3. Open Interest fell during the most recent minor dip

    Each check carries its own status: "pass", "fail", or "insufficient_data".
    """
    out = {
        "inst_id": inst_id,
    ...
    # ---- 1. Live price + funding rate (fresh OKX calls) -------------
    price = None
    try:
        tdata = GET(OKX + "/api/v5/market/ticker", params={"instId": inst_id})
        rows = tdata.get("data") or []
        if rows:
            price = float(rows[0].get("last") or 0)
    except Exception as e:
        out["errors"].append("ticker: " + str(e)[:80])

    funding = None
    try:
        fdata = GET(OKX + "/api/v5/public/funding-rate", params={"instId": inst_id})
        frows = fdata.get("data") or []
        if frows:
            funding = float(frows[0].get("fundingRate") or 0)
        else:
            out["errors"].append("funding: OKX returned no data for this instrument")
    except Exception as e:
        out["errors"].append("funding: " + str(e)[:80])

    out["price"]   = price
    out["funding"] = funding   # decimal, e.g. 0.0001 = 0.01%

    # ---- 2. Live 15m candles -> EMA(20) ------------------------------
    candles_15m = fetch_ohlcv_okx(inst_id, bar="15m", limit=120)
    ema20_last  = None
    ema_dist_pct = None
    if len(candles_15m) >= 20:
        closes = [c["c"] for c in candles_15m]
        ema_series = _ema(closes, 20)
        if ema_series:
            ema20_last = ema_series[-1]
            ref_price = price if price else closes[-1]
            if ema20_last:
                ema_dist_pct = (ref_price - ema20_last) / ema20_last * 100.0

    out["ema20_15m"]      = ema20_last
    out["ema_dist_pct"]   = ema_dist_pct
    out["candles_15m_n"]  = len(candles_15m)

    # ---- 3. Minor-dip detection from the same 15m candles ------------
    # Definition (stated plainly, not a hidden proprietary indicator):
    # look at the last 8 closed 15m candles (~2 hours). Find the highest
    # high in that window and the lowest low AFTER that high. If the low
    # comes after the high and represents a pullback of 0.5%-8% (a "minor"
    # dip, not a structural breakdown), that's the dip window we check OI
    # against. If price never pulled back, or pulled back more than 8%,
    # we report that plainly instead of forcing a verdict.
    dip_info = {"detected": False, "reason": None}
    if len(candles_15m) >= 8:
        window = candles_15m[-8:]
        highs  = [c["h"] for c in window]
        lows   = [c["l"] for c in window]
        hi_idx = highs.index(max(highs))
        if hi_idx < len(window) - 1:
            swing_high = highs[hi_idx]
            post_high_lows = lows[hi_idx:]
            low_idx_rel = post_high_lows.index(min(post_high_lows))
            swing_low   = post_high_lows[low_idx_rel]
            dip_pct = (swing_high - swing_low) / swing_high * 100.0 if swing_high else 0
            if 0.5 <= dip_pct <= 8.0:
                dip_info = {
                    "detected": True,
                    ...
                    "high_candle_idx_ago": len(window) - 1 - hi_idx,
                    "low_candle_idx_ago": len(window) - 1 - (hi_idx + low_idx_rel),
                }
            elif dip_pct > 8.0:
                dip_info["reason"] = ("Pullback is " + str(round(dip_pct, 1)) +
                                      "% -- too large to call a 'minor dip' " +
                                      "(structural move, not a healthy pause)")
            else:
                dip_info["reason"] = "No pullback >0.5% found in the last 2 hours"
        else:
            dip_info["reason"] = "Price still making new highs -- no dip yet to evaluate"
    else:
        dip_info["reason"] = "Not enough 15m candle history yet"

    out["dip"] = dip_info

    # ---- 4. OI during the dip window, from self-sampled history ------
    # We do NOT make a fresh OI call here for the historical comparison --
    # we read the same rolling window the scan loop already maintains
    # (sampled every SCAN_INTERVAL_SECONDS, default 5 min) so we don't
    # distort that cadence. We DO make one fresh call for the current
    # live OI snapshot, clearly labelled as "now".
    oi_now = None
    try:
        oidata = GET(OKX + "/api/v5/public/open-interest", params={"instId": inst_id})
        rows = oidata.get("data") or []
        if rows:
            oi_now = float(rows[0].get("oiCcy") or 0)
    except Exception as e:
        out["errors"].append("oi: " + str(e)[:80])

    with _OI_HISTORY_LOCK:
        hist = list(_OI_HISTORY.get(inst_id, []))   # [(timestamp, oi_usd), ...]

    out["oi_now"] = oi_now
    out["oi_history_samples"] = len(hist)
    out["oi_history_window_minutes"] = (
        round((hist[-1][0] - hist[0][0]) / 60.0, 1) if len(hist) >= 2 else 0
    )

    oi_check = {"status": "insufficient_data", "detail": None}
    if len(hist) < 2:
        oi_check["detail"] = ("Only " + str(len(hist)) + " OI sample(s) collected so far -- "
                              "need at least 2 scan cycles (~" +
                              str(int(SCAN_INTERVAL_SECONDS * 2 / 60)) +
                              " min after first launch) before a dip comparison is possible.")
    else:
        oldest_oi = hist[0][1]
        newest_oi = hist[-1][1]
        oi_change_pct = ((newest_oi - oldest_oi) / oldest_oi * 100.0) if oldest_oi else 0.0
        oi_check["oi_oldest"] = oldest_oi
        oi_check["oi_newest"] = newest_oi
        oi_check["oi_change_pct"] = round(oi_change_pct, 2)
        if dip_info.get("detected"):
            if oi_change_pct < 0:
                oi_check["status"] = "pass"
                oi_check["detail"] = ("OI fell " + str(round(abs(oi_change_pct), 2)) +
                                      "% across the sampled window during this dip -- " +
                                      "consistent with long-holders closing out, not new shorts piling in.")
            else:
                oi_check["status"] = "fail"
                oi_check["detail"] = ("OI rose " + str(round(oi_change_pct, 2)) +
                                      "% across the sampled window during this dip -- " +
                                      "consistent with aggressive new short positioning, not a healthy pause.")
        else:
            oi_check["status"] = "insufficient_data"
            oi_check["detail"] = "No qualifying minor dip detected in the price action right now, so there's nothing to check OI against."

    out["oi_check"] = oi_check

    # ---- 5. Funding rate check ----------------------------------------
    funding_check = {"status": "insufficient_data", "detail": None}
    if funding is not None:
        funding_pct = funding * 100.0
        if funding_pct < 0.05:
            funding_check["status"] = "pass"
            funding_check["detail"] = ("Funding rate is " + str(round(funding_pct, 4)) +
                                       "% per 8h -- below the 0.05% danger threshold, " +
                                       "so longs aren't being charged a punitive premium.")
        else:
            funding_check["status"] = "fail"
            funding_check["detail"] = ("Funding rate is " + str(round(funding_pct, 4)) +
                                       "% per 8h -- at or above the 0.05% threshold. " +
                                       "Longs are paying a heavy premium, raising the risk of a " +
                                       "crowded-long unwind.")
    else:
        funding_check["detail"] = "Funding rate could not be fetched live right now."
    out["funding_check"] = funding_check

    # ---- 6. EMA distance check -----------------------------------------
    ema_check = {"status": "insufficient_data", "detail": None}
    if ema_dist_pct is not None:
        abs_dist = abs(ema_dist_pct)
        if abs_dist <= 1.5:
            ema_check["status"] = "pass"
            side = "above" if ema_dist_pct >= 0 else "below"
            ema_check["detail"] = ("Price is " + str(round(abs_dist, 2)) + "% " + side +
                                   " the live 15m EMA(20) -- close enough to call this " +
                                   "a pullback-to-the-average zone, not an extended chase.")
        else:
            side = "above" if ema_dist_pct >= 0 else "below"
            ema_check["status"] = "fail"
            ema_check["detail"] = ("Price is " + str(round(abs_dist, 2)) + "% " + side +
                                   " the live 15m EMA(20) -- stretched further than the 1.5% " +
                                   "threshold, so a snap-back toward the average is still " +
                                   "mathematically likely.")
    else:
        ema_check["detail"] = "Not enough 15m candle history yet to compute EMA(20)."
    out["ema_check"] = ema_check

    # ---- 7. Overall verdict --------------------------------------------
    checks = [funding_check["status"], ema_check["status"], oi_check["status"]]
    if all(c == "pass" for c in checks):
        verdict = "SAFE_TRADE"
    elif any(c == "fail" for c in checks):
        verdict = "BREAKDOWN_RISK"
    else:
